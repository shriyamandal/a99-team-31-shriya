...
// import {default as ratings} from '@mtucourses/rate-my-professors';
import {searchSchool, searchTeacher} from '@mtucourses/rate-my-professors';
import rating from '@mtucourses/rate-my-professors';
const {searchSchool, searchTeacher, getTeacher} = rating;
...
const getRatingsForTeacher = async (teacherName) => {
    const possibleTeachers = await rating.searchTeacher(teacherName, uncID);
    if (!possibleTeachers.length) return {};

    const teacherId = possibleTeachers[0].id;
    const teacher = await rating.getTeacher(teacherId);
