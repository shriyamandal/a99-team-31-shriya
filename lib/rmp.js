...
import rate-my-professors from '@mtucourses/rate-my-professors';
...
const getRatingsForTeacher = async (teacherName) => {
    const possibleTeachers = await default.searchTeacher(teacherName, uncID);
    if (!possibleTeachers.length) return {};

    const teacherId = possibleTeachers[0].id;
    const teacher = await default.getTeacher(teacherId);
